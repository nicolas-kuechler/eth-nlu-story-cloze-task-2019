...
flags.DEFINE_string(
    "train_data_dir", None,
    "The input data dir of the training data. Should contain the .tsv files (or other data files) "
    "for the task.")
...
flags.DEFINE_bool(
    ...
    "Whether to run the model in inference mode on the cross product set.")
...
class DataProcessor(object):
  ...
  def get_cross_examples(self, data_dir):
...
class SCTProcessor(DataProcessor):
  """Processor for the Stroy Close data set."""

  def get_train_examples(self, data_dir):
    """See base class."""

    return self._create_examples(
        self._read_tsv(os.path.join(data_dir,"ds_train.tsv")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "ds_valid.tsv")),"dev")
  
  def get_cross_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "ds_cross_product_false.tsv")), "cross")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      story_start_id = tokenization.convert_to_unicode(line[0])
      story_end_id = tokenization.convert_to_unicode(line[1])
      story_start = tokenization.convert_to_unicode(line[2])
      story_end	= tokenization.convert_to_unicode(line[3])
      label = tokenization.convert_to_unicode(line[4])
      
      guid = "%s_%s_%s" % (set_type, story_start_id, story_end_id)

      examples.append(
          InputExample(guid=guid, text_a=story_start, text_b=story_end, label=label))
    return examples
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    scaffold_fn = None
    if init_checkpoint:
      tf.logging.info("Init Checkpoint: %s", init_checkpoint)
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    if not os.path.isfile(eval_file):
      ...
      tf.logging.info("***** Running evaluation *****")
      tf.logging.info("  restoring tf records from: %s", eval_file)
      tf.logging.info("  Batch size = %d", FLAGS.eval_batch_size)
  ...
  if FLAGS.do_predict_valid:
    predict_file = os.path.join(FLAGS.data_dir, "predict_valid.tf_record")
    ...
    if not os.path.isfile(predict_file):
      ...
      tf.logging.info("***** Running prediction on valid dataset*****")
      tf.logging.info("  Num examples = %d (%d actual, %d padding)",
    ...
    output_predict_file = os.path.join(FLAGS.output_dir, "valid_results.tsv")
  ...
  if FLAGS.do_predict_cross:
    ...
    if not os.path.isfile(predict_file):
      predict_examples = processor.get_cross_examples(FLAGS.data_dir)
      num_actual_predict_examples = len(predict_examples)
      if FLAGS.use_tpu:
        # TPU requires a fixed batch size for all batches, therefore the number
        # of examples must be a multiple of the batch size, or else examples
      ...
      tf.logging.info("***** Running prediction on cross prod dataset*****")
      tf.logging.info("  Num examples = %d (%d actual, %d padding)",
                      len(predict_examples), num_actual_predict_examples,
                      len(predict_examples) - num_actual_predict_examples)
      tf.logging.info("  Batch size = %d", FLAGS.predict_batch_size)
    ...
    output_predict_file = os.path.join(FLAGS.output_dir, "cross_results.tsv")
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      num_written_lines = 0
      tf.logging.info("***** Predict results *****")
      for (i, prediction) in enumerate(result):
